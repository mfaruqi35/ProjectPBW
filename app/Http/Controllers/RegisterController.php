...
<body>
    <div class="w-screen h-screen bg-[url(/assets/background.png)] flex justify-center items-center">
        <div class="shadow-lg p-20 rounded-3xl flex justify-center items-center flex-col bg-white relative">
            ...
            <div class="flex flex-col gap-5 w-full">
                ...
                <form action="{{ route('register') }}" method="POST" class="w-full">
                    ...
                    <input type="text" name="username" value="{{ old('username') }}" required autocomplete="name" autofocus
                           class="border-b-1 border-[#8c8c8c] rounded-sm w-[25vw] h-[6vh] p-2 focus:border-b-1 focus:border-[#1868DB] focus:outline-none @error('username') @enderror"
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autocomplete="email"
                           class="border-b-1 border-[#8c8c8c] rounded-sm w-[25vw] h-[6vh] p-2 mt-5 focus:border-b-1 focus:border-[#1868DB] focus:outline-none @error('email') @enderror"
                           placeholder="Email">
                    @error('email')
                        <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                    @enderror
                    ...
                    <input type="password" name="password_confirmation" required autocomplete="new-password"
                           class="border-b-1 border-[#8c8c8c] rounded-sm w-[25vw] h-[6vh] p-2 mt-5 focus:border-b-1 focus:border-[#1868DB] focus:outline-none"
            ...
            <a href="{{ route('login') }}" class="w-[25vw] flex justify-end">
