...
<body class="bg-gray-100 font-sans leading-normal tracking-normal">
    <nav class="bg-white shadow-lg p-4 flex justify-between items-center fixed w-full z-10 top-0">
        ...
        <div>
            <a href="{{ route('projects.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded">Back to Projects</a>
        </div>
    </nav>

    <div class="container mx-auto pt-20 px-6">
        <h2 class="text-3xl font-bold mb-6 text-gray-800">Edit Project: {{ $project->name }}</h2>
        ...
        <form action="{{ route('projects.update', $project) }}" method="POST" class="bg-white p-8 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            <div class="mb-4">
